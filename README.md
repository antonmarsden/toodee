...
The `CopyOps` trait provides various operations that copy data within the same 2D array, or copy data from one array to another. Many of these
...
Serialization and deserialization of `TooDee` objects.
`TooDeeView` and `TooDeeViewMut` can be serialized, but must be deserialized into a `TooDee` structure.
